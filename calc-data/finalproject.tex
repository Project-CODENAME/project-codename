...
\end{abstract}
\part{Common Tools}
For determining rates of change with discrete values, the difference quotient must be used between every point.  The difference quotient takes the form:
$$m=\frac{f(x+h)-f(x)}{h}$$
Where $h$ is the difference in value between two values on the x axis.
